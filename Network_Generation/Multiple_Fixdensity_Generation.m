...
clear all
filename = '../simulation_multi_fixden.mat';
...
e_1 = density_1*(N_1*(N_1-1))/2;%first choice of edges 
for i = 1:times
adj_1{i} = random_graph(N_1,[],e_1);%generate 20 random adjacency matrixes given number of nodes and edges
end

e_2 = density_2*(N_1*(N_1-1))/2; %second choice of edges = 10N
for i = 1:times
adj_2{i} = random_graph(N_1,[],e_2);
end

e_3 = density_3*(N_1*(N_1-1))/2;  %third choice of edges = full
...
e_4 = density_1*(N_2*(N_2-1))/2;
for i = 1:times
adj_4{i} = random_graph(N_2,[],e_4);
end

e_5 = density_2*(N_2*(N_2-1))/2; 
for i = 1: times
adj_5{i} = random_graph(N_2,[],e_5);
end

e_6 = density_3*(N_2*(N_2-1))/2;
...
e_7 = density_1*(N_3*(N_3-1))/2; 
for i = 1:times
adj_7{i} = random_graph(N_3,[],e_7);
end

e_8 = density_2*(N_3*(N_3-1))/2; 
for i = 1:times
adj_8{i} = random_graph(N_3,[],e_8);
end

e_9 = density_3*(N_3*(N_3-1))/2;
