...
%This code cite from https://github.com/IdeasLabUT/Dynamic-Stochastic-Block-Model/blob/master/generateSbm.m
%Author: Kevin S. Xu

function A = generateSbm(c,P,directed)
...
A = zeros(n,n); % Graph adjacency matrix
...
for c1 = 1:k
    ...
    if numC1 > 1
        if directed == true
            ...
            A(inC1,inC1) = squareform(bernrnd(P(c1,c1), ...
                numC1*(numC1-1)/2,1));
        end
    end
    ...
    for c2 = start:k
        ...
        A(inC1,inC2) = bernrnd(P(c1,c2),numC1,numC2);
        if directed == false
            A(inC2,inC1) = A(inC1,inC2)';
        end
    end
end
